...
# device = torch.device('cpu') # uncomment this line if cuda out of memory RuntimeError occurs
...
if platform.system() == 'Windows':
    answers_dir = Path('answers') / (datetime.now().strftime('%Y-%m-%d %H-%M-%S') + 'Z') # for WIN ':' tokens throw an error
else:
    answers_dir = Path('answers') / (datetime.utcnow().isoformat() + 'Z') # all other platforms
